...
def Googlesearch(query_google):
    if query_google:
        ...
        messagebox.showerror(
            "No Text Selected!",
            "Looks like you forgot to highlight any text. "
...
def calculate(self):
    ...
    try:
        res = int(eval(stringg))  # noqa: PGH001
        res = str(res)
...
def pastebin(self):
    ...
    response = (
        requests.post("https://pastebin.com/api/api_post.php", data=data, timeout=1000)
    ).text
    ...
    if sample_string:
        file = Path(
            filedialog.asksaveasfilename(
                ...
                defaultextension=codesnippet_ext,
            ),
        ).open("w")
        ...
        messagebox.showerror(
            "It's empty here!",
            "Hey there, mind selecting some text before trying to make changes? We can't read your mind... yet.",
        )
...
def save_document(self):
    try:
        file = Path(
            filedialog.asksaveasfilename(title="Select file", defaultextension=".py"),
        ).open("w")
