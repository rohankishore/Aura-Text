...
def main():
    ...
    # text box for entry
    notepad = Text(editor, font=("Segoe UI", 13,), height=100, foreground="white",
                   insertbackground="white", borderwidth=0, undo=True, background="#2b2b2b")

    scrollbar = ttk.Scrollbar(editor)
    scrollbar.pack(side=RIGHT, fill=Y)
    notepad.config(yscrollcommand=scrollbar.set, background="#2b2b2b")
    scrollbar.config(command=notepad.yview)

    notepad.pack(fill=BOTH)
    notepad.focus_set()

    code_editor = Text(coding, font=("Arial", 12), height=100, background="#2b2b2b", foreground="white",
    ...
    def codeSave():  # file menu Save As option
        fd = filedialog.asksaveasfile(mode='w', defaultextension='.py', initialfile="scratch")
        t = code_editor.get(0.0, END)  # t stands for the text gotten from notepad
        try:
            fd.write(t.rstrip())
    ...
    class LineNumbers(Text):
        ...
        def on_key_release(self, event=None):
            ...
            self.configure(state='normal', width=width, font=("Arial", 12), foreground="#d3d3d3", background="#313335")
            self.delete(1.0, END)
    ...
    def save():
        if tabController.index("current") == editor:
            cmdSaveAs()
        elif tabController.index("current") == coding:
            codeSave()

    def Summary():
        note_lines = str(notepad.index('end-1c').split('.')[0])
        note_char_count = str(len(notepad.get("1.0", 'end-1c')))
        code_lines = str(code_editor.index('end-1c').split('.')[0])
        code_char_count = str(len(code_editor.get("1.0", 'end-1c')))

        text = "NOTEPAD" + '\n' + "\n" + "Total Number of Lines: " + note_lines + "\n" + "Total Character Count: " + note_char_count + "\n" + "\n" + "\n" + "CODE EDITOR" + "\n" + "\n" + "Total Number of Lines: " + code_lines + "\n" + "Total Character Count: " + code_char_count
    ...
    menubar = Menu(coding, background='blue', fg='white')
    ...
    file.add_command(label="Save Code File", command=codeSave)
    ...
    cdg = ColorDelegator()
    cdg.tagdefs['COMMENT'] = {'foreground': 'grey', 'background': '#2b2b2b'}
    cdg.tagdefs['KEYWORD'] = {'foreground': 'cyan', 'background': '#2b2b2b'}
    cdg.tagdefs['BUILTIN'] = {'foreground': 'gold', 'background': '#2b2b2b'}
    cdg.tagdefs['STRING'] = {'foreground': 'light green', 'background': '#2b2b2b'}
    cdg.tagdefs['DEFINITION'] = {'foreground': 'gold', 'background': '#2b2b2b'}
    cdg.tagdefs['BRACKETS'] = {'foreground': '#007F7F', 'background': '#2b2b2b'}
    ...
    ln = LineNumbers(coding, code_editor, width=0)
